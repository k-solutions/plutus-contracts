...
import           Plutus.Contract                (AsContractError (_ContractError),
                                                 Contract, ContractError,
                                                 Endpoint, HasEndpoint, Promise,
                                                 adjustUnbalancedTx, endpoint,
                                                 logError, logInfo, mapError,
                                                 mkTxConstraints, selectList,
                                                 submitUnbalancedTx, type (.\/),
                                                 utxosAt)
...
tokenAccountContract
    ...
    => Contract w s e ()
tokenAccountContract = mapError (review _TokenAccountError) (selectList [redeem_, pay_, newAccount_])
  where
    redeem_ :: forall w s e.
               ( HasTokenAccountSchema s
               , AsTokenAccountError e
               )
            => Promise w s e ()
    redeem_ = endpoint @"redeem" @(Account, PaymentPubKeyHash) @_ @_ $ \(accountOwner, destination) -> do
        void $ redeem destination accountOwner
        tokenAccountContract
    pay_ = endpoint @"pay" @_ @_ @_ $ \(accountOwner, value) -> do
        void $ pay accountOwner value
        tokenAccountContract
    newAccount_ = endpoint @"new-account" @_ @_ @_ $ \(tokenName, initialOwner) -> do
...
redeemTx :: forall w s e.
    ...
    => Account
    -> PaymentPubKeyHash
    -> Contract w s e (Maybe (TxConstraints () (), ScriptLookups TokenAccount))
    ...
    case Map.keys utxos of
            ...
            oref : _ -> do
              let constraints :: TxConstraints () ()
                  constraints =  Constraints.mustSpendScriptOutput oref ()
...
redeem pk account = mapError ( review _TokenAccountError) $ do
    mbRes <- redeemTx account pk
    case mbRes of
      Just (constraints, lookups) -> mkTxConstraints lookups constraints >>= adjustUnbalancedTx >>= submitUnbalancedTx
      Nothing                     -> logError @String "No Unspent output"
